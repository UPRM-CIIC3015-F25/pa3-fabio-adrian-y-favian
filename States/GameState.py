...
class GameState(State):
    ...
    def calculate_gold_reward(self, playerInfo, stage=0):
        ...
        recompensa_base = {'SMALL':4, 'BIG':8, 'BOSS': 10}
        oro_base = recompensa_base.get(blind_type, 4)
        ...
        if stage == 1:
            ...
            overkill_restante = score - (target * 5)
            if bonus < 5 and overkill_restante > 0:
                return bonus + self.calculate_gold_reward(playerInfo, 1) #el 1 vendria siendo el stage
            else:
                return bonus

        #The following line of code would be the recursion when stage is 0
        #starts with base reward and begins overkill calculation
        return oro_base + self.calculate_gold_reward(playerInfo, 1)
    ...
    def SortCards(self, sort_by: str = "suit"):
        ...
        def swap_card(card1, card2):
            self.hand[card1], self.hand[card2] = self.hand[card2], self.hand[card1]

        def is_rank_lower(card1, card2):
            if self.hand[card1].rank.value < self.hand[card2].rank.value:
                return True

        def is_suit_lower(card1, card2):
            if suitOrder.index(self.hand[card1].suit) < suitOrder.index(self.hand[card2].suit):
                return True

        for curr in range(hand_length):
            for rest in range(curr + 1, hand_length):
                if sort_by == 'suit':
                    if is_suit_lower(curr, rest):
                        swap_card(curr, rest)
                    elif suitOrder.index(self.hand[curr].suit) == suitOrder.index(self.hand[rest].suit):
                        if is_rank_lower(curr, rest):
                            swap_card(curr, rest)
                elif sort_by == 'rank':
                    if is_rank_lower(curr, rest):
                        swap_card(curr, rest)
                    elif self.hand[curr].rank.value == self.hand[rest].rank.value:
                        if is_suit_lower(curr,rest):
                            swap_card(curr, rest)
        self.updateCards(400, 520, self.cards, self.hand, scale=1.2)
    ...
    def discardCards(self, removeFromHand: bool):
        if len(self.cardsSelectedList) == 0:
            cards_to_be_drawn = 8 -len(self.hand)
            if cards_to_be_drawn > 0:
                self.drawNewCards(cards_to_be_drawn)
    ...
    def drawNewCards(self, count):
        if count <= 0:
            return
        if len(self.deck) > 0:
            card = self.deck.pop(0)
            self.hand.append(card)
        self.drawNewCards(count - 1)
